...
#exec "$JAVA" -Xms128M -Xmx512M -classpath "$CLASSPATH"  com.yunling.mediacenter.server.WatchServer "$@" >> start-log`date +%Y%m%d`.txt 2>&1
exec "$JAVA" -Xms128M -Xmx1024M -classpath "$CLASSPATH"  com.yunling.mediacenter.server.WatchServer "$@" 